...
class ResCompany(models.Model):
    ...
    rea_office = fields.Many2one('res.country.state', string='Office Province', readonly=False)
    rea_code = fields.Char('REA Code', size=20, readonly=False)
    rea_capital = fields.Float('Share Capital', readonly=False)
    rea_member_type = fields.Selection([('SU', 'Unique Member'),
                                        ('SM', 'Multiple Members')], 'Member Type', readonly=False)
    rea_liquidation_state = fields.Selection([('LS', 'In liquidation'),
                                              ('LN', 'Not in liquidation')], 'Liquidation State', readonly=False)
    ...
    def onchange_rea_data(self):
        ...
        rea_member_type = ''
        if any(self.rea_office, self.rea_code, self.rea_capital, self.rea_member_type, self.rea_liquidation_state):
